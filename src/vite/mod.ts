...
export async function sveltekit_modal(
  ...
  const plugin_glob_sync: Plugin = globsync({
    globs: ["src/routes/**/*.py", "sveltekit_modal_config.py"],
  ...
  const plugin_modal_serve: Plugin = {
    ...
    async configureServer({ config }) {
      ...
      local_process.stdout.on("data", (s) => {
        ...
        const modal_route_match = out
          .match(/https:\/\/.*?\.modal\.run/)
          ?.toString();
    ...
    transform(src, id) {
      if (/\.py$/.test(id)) {
        if (modal_url === undefined)
          throw new Error(
            `${plugin_modal_serve.name} failed to produce a modal_url`
          );
